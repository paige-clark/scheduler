...
import { getInterviewersForDay } from './getInterviewersForDay';
...
test('getInterviewersForDay returns an array containing the correct interviewers objects', () => {
  const [first, second] = getInterviewersForDay(state2, 'Tuesday');
  expect(first).toEqual(state2.interviewers['1']);
...
test('getInterviewersForDay returns an empty array when the day is not found', () => {
  const result = getInterviewersForDay(state2, 'Wednesday');
  expect(result.length).toEqual(0);
});
